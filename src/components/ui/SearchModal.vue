...
<template>
  <Teleport to="body">
    <Transition name="fade-backdrop">
      <div 
        v-if="isOpen" 
        class="fixed inset-0 z-50 flex items-start justify-center pt-16 sm:pt-24 px-4 bg-primary/60 backdrop-blur-xs"
        @click.self="close"
      >
        <Transition name="modal-zoom">
          <div 
            v-if="isOpen"
            class="bg-surface w-full max-w-2xl border border-outline-variant rounded-2xl shadow-2xl p-6 md:p-8 overflow-hidden"
          >
            <!-- Search Input Header: Píldora -->
            <div class="relative flex items-center bg-surface-container rounded-full px-4 py-3 border border-outline-variant focus-within:border-primary mb-6 transition-all shadow-xs">
              ...
              <button 
                @click="close"
                class="text-xs font-label uppercase tracking-widest text-secondary hover:text-primary transition-all bg-surface border border-outline-variant px-3 py-1 rounded-full hover:scale-105 active:scale-95"
              >
                ESC
              </button>
            </div>
            ...
            <div class="mb-6">
              ...
              <div class="flex flex-wrap gap-2">
                <button
                  ...
                  class="text-xs font-label px-3.5 py-1.5 bg-surface-container hover:bg-secondary-container text-primary transition-all duration-200 border border-outline-variant rounded-full hover:scale-105 active:scale-95 shadow-2xs"
                >
                  {{ tag }}
                </button>
              </div>
            ...
            <div>
              <div class="flex justify-between items-center mb-3">
                ...
                <RouterLink 
                  ...
                  @click="close"
                  class="text-xs font-label uppercase text-primary underline hover:text-primary-container transition-colors"
                >
                  Ver todos en catálogo
                </RouterLink>
              </div>

              <div v-if="filteredProducts.length > 0" class="divide-y divide-outline-variant/60 max-h-80 overflow-y-auto pr-2">
                <div
                  v-for="product in filteredProducts"
                  :key="product.id"
                  @click="selectProduct(product.slug)"
                  class="flex items-center gap-4 py-3 hover:bg-surface-container px-3 rounded-xl cursor-pointer transition-all duration-200 group hover:translate-x-1"
                >
                  <img 
                    :src="product.images[0]" 
                    :alt="product.name"
                    class="w-12 h-16 object-cover bg-surface-lowest rounded-lg border border-outline-variant flex-shrink-0 transition-transform duration-300 group-hover:scale-105"
                  />
              ...
              <div v-else class="py-12 text-center text-secondary">
                <span class="material-symbols-outlined text-4xl mb-2 text-outline animate-float-gentle">search_off</span>
