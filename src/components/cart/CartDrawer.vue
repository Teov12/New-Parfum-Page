  ...
  <Teleport to="body">
    ...
    <Transition name="fade-backdrop">
      <div 
        ...
        class="fixed inset-0 z-50 bg-primary/50 backdrop-blur-xs"
    ...
    <Transition name="slide-drawer">
      <aside 
        v-if="cartStore.isDrawerOpen"
        class="fixed top-0 right-0 z-50 h-full w-full max-w-md bg-surface border-l border-outline-variant shadow-2xl flex flex-col overflow-hidden"
      >
        <!-- Header -->
        <div class="p-6 border-b border-outline-variant flex justify-between items-center bg-surface">
          ...
          <button 
            @click="cartStore.closeDrawer"
            class="w-10 h-10 rounded-full hover:bg-surface-container flex items-center justify-center text-primary transition-colors hover:scale-105 active:scale-95"
        ...
        <div class="px-6 py-3.5 bg-surface-container border-b border-outline-variant">
          ...
          <div class="w-full bg-secondary-container h-2 rounded-full overflow-hidden">
            <div 
              class="bg-primary-container h-full rounded-full transition-all duration-700 ease-out"
        ...
        <div class="flex-grow overflow-y-auto p-6 space-y-4">
          <div v-if="cartStore.items.length === 0" class="h-full flex flex-col items-center justify-center text-center py-16">
            <span class="material-symbols-outlined text-5xl text-outline mb-4 animate-float-gentle">shopping_bag</span>
            ...
            <button 
              @click="cartStore.closeDrawer(); router.push('/catalogo')"
              class="bg-primary-container text-on-primary font-label text-label-sm px-8 py-3.5 uppercase tracking-widest rounded-full hover:bg-inverse-surface transition-all shadow-xs hover:shadow-md active:scale-95"
            >
              Explorar Catálogo
            </button>
          </div>
          ...
          <TransitionGroup v-else name="cart-item" tag="div" class="space-y-4">
            <div 
              ...
              class="p-4 bg-surface-container-low rounded-xl border border-outline-variant flex gap-4 transition-all duration-300 hover:shadow-sm"
            >
              <!-- Thumbnail -->
              <img 
                :src="item.image" 
                :alt="item.name"
                class="w-20 h-24 object-cover bg-surface-lowest rounded-lg border border-outline-variant flex-shrink-0"
              />
                ...
                <div>
                  <div class="flex justify-between items-start gap-2">
                    ...
                    <button 
                      ...
                      class="text-secondary hover:text-error hover:scale-110 active:scale-90 transition-all p-1"
                ...
                <div class="flex justify-between items-center mt-3 pt-2">
                  <div class="inline-flex items-center border border-outline-variant rounded-full bg-surface overflow-hidden shadow-2xs">
                    <button 
                      ...
                      class="px-3 py-1 text-primary hover:bg-surface-container active:scale-90 transition-all text-sm font-bold"
        ...
        <div v-if="cartStore.items.length > 0" class="p-6 bg-surface border-t border-outline-variant space-y-4">
          ...
          <div class="grid grid-cols-2 gap-3 pt-2">
            <button 
              @click="goToCartPage"
              class="w-full bg-transparent text-primary font-label text-label-sm py-3.5 uppercase tracking-widest rounded-full border border-primary hover:bg-surface-container transition-all text-center shadow-2xs hover:shadow-sm active:scale-95"
            >
              Ver Carrito
            </button>
            <button 
              @click="goToCheckout"
              class="w-full bg-primary-container text-on-primary font-label text-label-sm py-3.5 uppercase tracking-widest rounded-full border border-primary-container hover:bg-inverse-surface transition-all text-center flex items-center justify-center gap-2 shadow-md hover:shadow-xl active:scale-95"
            >
              <span>Checkout</span>
